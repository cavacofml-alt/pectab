...
const I18N = {
  pt: {
    ...
    "stock.sourceNote": "Referência genérica (IATA Resolution 740 e fornecedores de stock), não específica de nenhum cliente ou PECTAB — nunca substitui o catálogo. Fontes secundárias, via resumo de pesquisa web; confirma se for crítico.",
    "stock.width.ok": "Largura {width}mm está dentro do intervalo padrão da indústria ({min}-{max}mm)",
    "stock.width.out": "Largura {width}mm está FORA do intervalo padrão da indústria ({min}-{max}mm) — confirma a medida",
    "stock.length.knownMatch": "Comprimento corresponde a um tamanho de stock comercial conhecido: {label} ({note})",
    "stock.length.inRange": "Comprimento {len}mm está dentro do intervalo típico da indústria ({min}-{max}mm)",
    "stock.length.outRange": "Comprimento {len}mm está fora do intervalo típico da indústria ({min}-{max}mm) — não é necessariamente um erro, mas é menos comum",
  ...
  en: {
    ...
    "stock.sourceNote": "Generic reference (IATA Resolution 740 and stock suppliers), not specific to any customer or PECTAB — never a substitute for the catalog. Secondary sources, via web search summary; verify if it matters.",
    "stock.width.ok": "Width {width}mm is within the industry standard range ({min}-{max}mm)",
    "stock.width.out": "Width {width}mm is OUTSIDE the industry standard range ({min}-{max}mm) — double-check the measurement",
    "stock.length.knownMatch": "Length matches a known commercial stock size: {label} ({note})",
    "stock.length.inRange": "Length {len}mm is within the typical industry range ({min}-{max}mm)",
    "stock.length.outRange": "Length {len}mm is outside the typical industry range ({min}-{max}mm) — not necessarily an error, just less common",
