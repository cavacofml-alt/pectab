...
const I18N = {
  pt: {
    ...
    "label.width": "largura (mm) — opcional",
    "field.width.tooltip": "Largura da etiqueta (eixo perpendicular ao comprimento). Não entra no motor de matching de PECTABs — só serve para o confronto com stocks conhecidos da indústria, abaixo.",
    "stock.title": "Confronto com stocks conhecidos da indústria",
    "stock.sourceNote": "Referência genérica, não específica de nenhum cliente ou PECTAB — nunca substitui o catálogo. Largura: IATA Resolution 740, confirmado por convergência entre duas pesquisas de IA independentes (doc. primário não acedido diretamente). Tamanhos de comprimento: produtos comerciais reais, não definidos pela IATA.",
    "stock.width.ok": "Largura {width}mm está dentro do intervalo padrão IATA ({min}-{max}mm)",
    "stock.width.out": "Largura {width}mm está FORA do intervalo padrão IATA ({min}-{max}mm) — confirma a medida",
    "stock.length.knownMatch": "Comprimento corresponde a um tamanho comercial conhecido: {label} ({note})",
    "field.eqBadge.title": "talões não são todos iguais",
    "field.eqBadge.titleKnown": "talões não são todos iguais — valores reais: {values}mm",
    ...
    "option.dirUnknown": "Desconhecida — testar as duas",
    ...
    "equiv.title": "⚠ {count} PECTABs fisicamente indistinguíveis com esta medida — a medida física não consegue escolher entre eles, confirma qual usar por outro critério (destino, remarks, quem pediu)",
    "checklist.title": "Porque bate certo",
    "checklist.dir": "Direção",
    "checklist.dir.hypothesis": "hipótese: {dir} primeiro",
    "results.dirUnknown.note": "🔀 Direção desconhecida — os resultados combinam candidatos PAX-primeiro e ADD-primeiro. Cada candidato mostra qual hipótese assume.",
    ...
    "warn.eqNKnown": "Este PECTAB tem talões de tamanhos diferentes (eq=N) — valores reais, por ordem: {values}mm.",
    ...
    "toast.loadResultUpdated": "{label}: {added} adicionados, {updated} atualizados.",
  ...
  en: {
    ...
    "label.width": "width (mm) — optional",
    "field.width.tooltip": "Tag width (axis perpendicular to length). Not used by the PECTAB matching engine — only used for the industry stock check below.",
    "stock.title": "Industry stock standards check",
    "stock.sourceNote": "Generic reference, not specific to any customer or PECTAB — never a substitute for the catalog. Width: IATA Resolution 740, confirmed by convergence across two independent AI research passes (primary document not directly accessed). Length sizes: real commercial products, not IATA-defined.",
    "stock.width.ok": "Width {width}mm is within the IATA standard range ({min}-{max}mm)",
    "stock.width.out": "Width {width}mm is OUTSIDE the IATA standard range ({min}-{max}mm) — double-check the measurement",
    "stock.length.knownMatch": "Length matches a known commercial stock size: {label} ({note})",
    "field.eqBadge.title": "stubs are not all the same size",
    "field.eqBadge.titleKnown": "stubs are not all the same size — real values: {values}mm",
    ...
    "option.dirUnknown": "Unknown — test both",
    ...
    "equiv.title": "⚠ {count} PECTABs are physically indistinguishable at this measurement — the physical measurement alone can't pick between them, confirm which to use by another criterion (destination, remarks, who requested it)",
    "checklist.title": "Why this matches",
    "checklist.dir": "Direction",
    "checklist.dir.hypothesis": "hypothesis: {dir} first",
    "results.dirUnknown.note": "🔀 Unknown direction — results combine PAX-first and ADD-first candidates. Each candidate shows which hypothesis it assumes.",
    ...
    "warn.eqNKnown": "This PECTAB has additional stubs of different sizes (eq=N) — real values, in order: {values}mm.",
    ...
    "toast.loadResultUpdated": "{label}: {added} added, {updated} updated.",
